...
int smscusbnet_stop (struct net_device *net)
{
    ...
    //if thera is in suspend0 status already, resume it first. So we can set suspend2 in Smsc7500_suspend();
...
static int smscusbnet_xmit (struct sk_buff *skb, struct net_device *net)
{
	...
	unsigned long		flags;
		 ...
		 */
		if (!(dev->driver_info->flags & FLAG_SEND_ZLP) && (skb->len % dev->maxpacket) == 0) {
...
#if 0 /* mergeing smsc7500usbnet and smsclan7500 */
