...
static int netdetach=0;
module_param(netdetach,bool, 0);
...
//Control message is very slow so it really isn't big deal to dynamically allocate the data
	...
	if (retVal<0){
		SMSC_WARNING("Failed to read register regAddr 0x%08x, set flag to recover", regAddr);
...
//Control message is very slow so it really isn't big deal to dynamically allocate the data
    ...
    *buf = data;
	...
	if (retVal<0){
		SMSC_WARNING("Failed to write register regAddr 0x%08x, set flag to recover", regAddr);
        set_bit (EVENT_DEV_RECOVERY, &dev->flags);
    }
...

{
    ...
    BUG_ON(!dev);
     ...
     if(mutex_lock_interruptible(&adapterData->phy_mutex)){
        return -EINTR;
     }
...
DONE:
    mutex_unlock(&adapterData->phy_mutex);
	return retVal;
...
static int read_data_port(struct usbnet *dev, int ramSel, u32 startAddr, u32 length, u32 *val)
{
    ...
    if(mutex_lock_interruptible(&adapterData->internal_ram_mutex)){
        return -EINTR;
     }
...
DONE:
    ...
    mutex_unlock(&adapterData->internal_ram_mutex);
...
static int smsc7500_rx_setmulticastlist(struct usbnet *dev)
{
	...
	if(mutex_lock_interruptible(&adapterData->RxFilterLock)){
       return -EINTR;
    }
	...
	mutex_unlock(&adapterData->RxFilterLock);
...
//                printk("%s():%d:Virgin value is %x\n", __FUNCTION__, __LINE__, dwValue);
		...
		bcr = 0;
		if(dwValue & (LINK_SPEED_MASTER_1000FD | LINK_SPEED_SLAVE_1000FD)){
			...
			/* Force Master/Slave Manual Config */
...
static int smsc7500_bind(struct usbnet *dev, struct usb_interface *intf)
{
    ...
    mutex_init(&adapterData->phy_mutex);
    mutex_init(&adapterData->internal_ram_mutex);
    mutex_init(&adapterData->RxFilterLock);
...
#if defined(CONFIG_MACH_VAR_SOM_OM4460)
	if (machine_is_var_som_om4460()) {
		extern u8 *vatiscite_blob_get_eth_mac_base(void);
		u8 *boot_mac_addr  = vatiscite_blob_get_eth_mac_base();

		/* set Variscite's ON SOM Ethernet interface MAC from on SOM EEROM */
		if (boot_mac_addr) {

			adapterData->macAddrHi16 = *(u16*)(boot_mac_addr + sizeof(u32));
			adapterData->MmacAddrLo32 = *(u32*)boot_mac_addr;

			SMSC_DEBUG(DBG_INIT,"MAC address set from Variscite SOM EEPROM");
		}
	}
#endif		
...
#ifdef CONFIG_PM
	if(dev->dynamicSuspend || dev->linkDownSuspend){
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,21) && (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,32)))
...
static struct sk_buff *smsc7500_tx_fixup(struct usbnet *dev, struct sk_buff *skb, int flags)
{
	...
	if(skb_is_nonlinear(skb)){
		...
		skbSize = skb->len + TX_TOTAL_CONTROL_WORD_LEN;
		skb_tmp = dev_alloc_skb(skbSize);
		if (!skb_tmp){
			SMSC_WARNING("cannot allocat skb buffer\n");
			return NULL;
		}
		skb_put(skb_tmp, skbSize - TX_TOTAL_CONTROL_WORD_LEN);
	...
	if(headroom < TX_TOTAL_CONTROL_WORD_LEN){

		if (!skb_cloned(skbOut)){
			 if((headroom + tailroom) >= TX_TOTAL_CONTROL_WORD_LEN){
		...
		if(skbExpend){
			...
			skb_tmp = skb_copy_expand(skbOut, TX_TOTAL_CONTROL_WORD_LEN, 0, flags);
			if (!skb_tmp){
				return NULL;
			}
...
/*********************Init Burst delay ******************************************************/
	...
	if ((retVal = smsc7500_write_reg(dev, BULK_IN_DLY, DEFAULT_BULK_IN_DELAY))<0)
	{
