...
/* Variscite touch panel manual collaboration values */
#define VARISCITE_TS_X_MIN 120
#define VARISCITE_TS_X_MAX 3960
#define VARISCITE_TS_Y_MIN 280
#define VARISCITE_TS_Y_MAX 3850
...
static void tsc2004_work(struct work_struct *work)
{
	...
	if (rt) {
		...
		if (tc.x < VARISCITE_TS_X_MIN)
			tc.x = VARISCITE_TS_X_MIN;
		if (tc.x > VARISCITE_TS_X_MAX)
			tc.x = VARISCITE_TS_X_MAX;
		if (tc.y < VARISCITE_TS_Y_MIN)
			tc.y = VARISCITE_TS_Y_MIN;
		if (tc.y > VARISCITE_TS_Y_MAX)
			tc.y = VARISCITE_TS_Y_MAX;
				   ...
				   const struct i2c_device_id *id)
{
    ...
    printk(" tsc2004_probe \n");

	if (!pdata) {
		dev_err(&client->dev, "platform data is required!\n");
		return -EINVAL;
	}
    printk("b i2c_check_functionality \n");

	if (!i2c_check_functionality(client->adapter,
				     I2C_FUNC_SMBUS_READ_WORD_DATA))
		return -EIO;

    dev_dbg(&ts->client->dev, "tsc2004_probe check functionality\n");
	...
	input_set_abs_params(input_dev, ABS_X, VARISCITE_TS_X_MIN, VARISCITE_TS_X_MAX, 0, 0);
	input_set_abs_params(input_dev, ABS_Y, VARISCITE_TS_Y_MIN, VARISCITE_TS_Y_MAX, 0, 0);
